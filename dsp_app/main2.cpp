...
Issues covered: #5 - Thread-Safe Circular Buffer Integration
                ...
                #7 - SoundTouch DSP Integration( Focus of this code file)
...
Brief:
    - Implements multithreaded WAV playback using libsndfile and ALSA (wav_parser.c + alsa_playback.c)
      with real-time DSP processing using SoundTouch 
      and runtime control via a TCP socket server 
    ...
    - Uses a thread-safe circular (ring) buffer to decouple file I/O and playback (main1.c)
    - Producer thread reads audio frames from WAV file into buffer
    - Consumer thread processes audio through SoundTouch DSP and writes to ALSA PCM device
    - Integrates SoundTouch for real-time tempo and pitch control
    - Uses mutex-protected shared configuration for DSP parameters (tempo, pitch)
    - Control thread runs a TCP server to receive runtime commands (set tempo/pitch)
...
Note: SoundTouch is a native C++ library, and its C wrapper is incomplete/unreliable in our Buildroot setup.
      Missing functions like 16-bit processing APIs for exanple, caused linking issues which made writing this file in 
...
soundtouch::SoundTouch *pSoundTouch; // Native C++ DSP Object
...
void *consumer_thread_func(void *arg) {
    ...
    while (1) {
        ...
        // --- DSP PIPELINE (Static 1.0x Pass-Through for Issue #7) ---
 
        // 1. DSP BRIDGE IN: Convert 16-bit ints to 32-bit floats (-1.0 to 1.0)
        for (int i = 0; i < total_samples; i++) {
            float_in_buf[i] = (float)temp_buf[i] / 32768.0f;
        }

        // 2. Push normalized float frames to the DSP
        pSoundTouch->putSamples(float_in_buf, frames_to_pull);

        // 3. Continuously drain processed float frames from the DSP
        int processed_frames;
        while ((processed_frames = pSoundTouch->receiveSamples(float_out_buf, CHUNK_FRAMES)) > 0) {
            
            // 4. DSP BRIDGE OUT: Convert 32-bit floats back to 16-bit ints for ALSA
            ...
            // 5. Write the converted data to the ALSA hardware loop
...
void *control_thread_func(void *arg) {
    ...
    while (!ring_buf.eof_reached) {
        ...
        if (valread > 0) {
            ...
            // Parse the incoming string(this is the command)
	    // For this implementation we are testing tempo and pitch commands.
...
int main(int argc, char *argv[]) {
    ...
    ring_buf.data = (short *)malloc(ring_buf.max_frames * ring_buf.channels * sizeof(short)); // Added casting
    ...
    // Starting the DSP engine (using C++ since the library is written natively in C++) ---
    ...
    // Blocks main execution until the primary operations threads terminate
    ...
    // Execute the shutdown of hardware peripherals, memory, and variables
